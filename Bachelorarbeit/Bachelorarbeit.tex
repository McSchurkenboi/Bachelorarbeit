...
\caption{Formatierung des Textes für Tyü-65c-Boilerplates in \textit{formatBP()}-Methode [eigene Darstellung]}
\end{center}
\end{figure}
...
Die prototypische Interaktion verschiedener Nutzergruppen mit NLP-Werkzeugen wurde bereits in \cite{rs18} skizziert. Das dort beschriebene Aktivitätsdiagramm dient dabei zur Visualisierung der Interaktionspunkte mit dem Werkzeug von Anfang bis Ende der Nutzung. Die enthaltenen Interaktionspunkte haben sich in einer spezifischen GUI für das Werkzeug manifestiert. Werden die dort enthaltetenen Aktivitäten auf das in dieser Arbeit vorgestellte Werkzeug angewandt, ergibt sich folgende Konkretisierung der Aktivitäten, die ein Nutzer (hier RE-Experte) mit dem Tool hat:
...
\node[activity](e) at (-4, -11) {Satz überspringen};
\node[activity](f) at (0, -11) {Vorschlag anpassen};
\node[activity](g) at (4, -11) {Konvertierung übernehmen};
\node[dia](d1) at (0,-5.5){ };
\node[dia](d2) at (0,-9){ };
\node[dia](d3) at (0,-12.5){ };
\node[dia](d4) at (-7,-14){ };
...
\draw[->] (d1) to (c);
\draw[->] (c) to (d2);
\draw[->] (d2) to node[xshift=1.75cm, text width = 3cm] {[Fehlerhafter Vorschlag]} (f);
\draw[->] (d2) -| node[xshift=-0.5cm, yshift=0.5cm, text width = 3.5cm] {[keine Boilerplate gefunden]} (e);
\draw[->] (d2) -| node[xshift=0.75cm, yshift=0.5cm, text width = 3cm] {[Korrekte Konvertierung]} (g);
\draw[->] (d4) |- node[yshift=0.5cm, xshift=2.25cm] {[Alle Sätze verarbeitet]} (d);
\draw[->] (d4) |- node[xshift=2cm, yshift=0.5cm] {[Nächsten Satz bewerten]} (d1);
...
Für ein besseres Verständnis wird sich hier auf ein kleines Beispiel beschränkt, ausführliche Tests mit realen Lastenheften aus dem betrieblichen Alltag werden unter 6.x behandelt. Die Implementierung der Nutzerinteraktion wird daher anhand der Konvertierung der folgenden Sätze demonstriert, die in einem eingelesenen Lastenheft enhalten sein könnten:
\tt \\
...
\\ \rm
Dieses prototypische Lastenheft wird im Laufe dieses Abschnittes von natürlicher Sprache in formale Boilerplates konvertiert. Die sechs Beispielanforderungen sind dabei an betriebliche Produktspezifikationen, z.B. an ein Auto \textit{Golf7}, angelehnt. Jede Anforderung repräsentiert konkrete Szenarien, die im Programm bei der Analyse und Konvertierung auftreten können. Dies beinhaltet sowohl die Boilerplates, die für jeden Satz infrage kommen, als auch typische Fehler bei der Erkennung, die bei der Verarbeitung entsteht.
